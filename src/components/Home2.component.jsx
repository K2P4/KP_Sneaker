...
const Home2Component = ({ item: { id, image, name, price } }) => {
	...
	return (
		<motion.div
			...
			className="sm:w-[30%] w-full group  relative shadow-lg  h-auto py-2 px-4 bg-slate-200 rounded-lg hover:shadow-gray-500  duration-200">
			<div className="flex flex-col items-center align-middle ">
				<img
					className=" w-full -rotate-45 sm:h-[200px] rounded-md object-contain "
					src={image}
					alt=""
				/>
				...
				<h1 className="sm:text-lg  text-xs  sm:h-16 text-center font-semibold">{name}</h1>
				<p className="sm:text-md  text-xs mt-2 sm:mt-0 font-medium text-gray-700 ">$ {price}</p>
				<div className="flex items-center gap-1">
					{stars.map((star) => (
						<svg
							...
							viewBox="0 0 24 24"
							fill="currentColor"
							className="sm:w-6 w-3 h-3 text-orange-400 my-2 sm:h-6">
							<path
								fillRule="evenodd"
